...
class PHPCompatibility_Sniffs_PHP_ForbiddenNamesAsInvokedFunctionsSniff extends PHPCompatibility_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->supportsAbove($version)) {
            ...
            if($tokenContentLc == 'clone') {
                $fix   = $phpcsFile->addFixableError($error, $stackPtr, 'Found', $data);
                
                if ($fix === true) {
                    $startPos = $phpcsFile->findNext(T_OPEN_PARENTHESIS, $stackPtr);
                    $endPos   = $tokens[$startPos]['parenthesis_closer'];
                    $phpcsFile->fixer->replaceToken($startPos,' ');
                    $phpcsFile->fixer->replaceToken($endPos,'');
                }    
            }
            else {
